<div class="card card-navy" xmlns:wire="http://www.w3.org/1999/xhtml">
    ...
    <div class="card-header">
        ...
        <div class="card-tools">
            <button type="button" class="btn btn-tool" wire:click="setLimit" @if($rows > $listarSedes->count()) disabled @endif >
    ...
    <div class="card-body table-responsive p-0" @if($tableStyle) style="height: 72vh;" @endif>
        ...
        <table class="table table-head-fixed table-hover text-nowrap sticky-top">
            ...
            <tr class="text-navy">
                ...
                <th>
                    Descripción
                    <small class="float-right">Mostrando {{ $listarSedes->count() }}</small>
                </th>
            </tr>
            </thead>
        </table>
        ...
        <ul class="todo-list" data-widget="todo-list">
            @if($listarSedes->isNotEmpty())
                @foreach($listarSedes as $sede)
                    <li class=" @if(!$sede->estatus) done @endif @if($sede->id == $sedes_id) text-warning @endif " >
                        <!-- todo text -->
                        <span class="text">
                                {{ $sede->codigo }}
                        </span>
                        ...
                        <div class="tools text-primary" wire:click="show({{ $sede->id }})">
